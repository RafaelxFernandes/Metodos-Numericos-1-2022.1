...
# Classe para o elemento Caminho
class PATH:

    # Inicialização da classe
    def __init__(self, lengths, ctypes, L, x, y, yaw, directions):
        # Comprimentos de cada parte do caminho
        self.lengths = lengths  
        
        # Tipo de cada parte do caminho
        self.ctypes = ctypes  
        
        # Comprimento total do caminho
        self.L = L  
        
        # Posições finais em X
        self.x = x  
        
        # Posições finais em Y
        self.y = y  
        
        # Ângulos finais das curvas
        self.yaw = yaw  
        
        # Direções (1: frente, -1: trás)
...
def set_path(paths, lengths, ctypes):
    ...
    # Checa se o mesmo caminho já existe
    for path_e in paths:
...
# - Caminho STRAIGHT -> CURVE -> STRAIGHT
...
# - Caminho STRAIGHT -> LEFT -> STRAIGHT
def SLS(x, y, phi):
    phi = M(phi)
...
# - Caminho LEFT -> RIGHT -> LEFT -> RIGHT para v em calc_tauOmega igual à -u
def LRLRn(x, y, phi):
    xi = x + math.sin(phi)
...
# - Caminho CURVE -> CURVE -> CURVE -> CURVE
...
# - Caminho LEFT -> RIGHT -> STRAIGHT -> RIGHT
def LRSR(x, y, phi):
    xi = x + math.sin(phi)
    eta = y - 1.0 - math.cos(phi)
    rho, theta = R(-eta, xi)
...
# - Caminho LEFT -> RIGHT -> STRAIGHT -> LEFT
def LRSL(x, y, phi):
    xi = x - math.sin(phi)
    eta = y - 1.0 + math.cos(phi)
    rho, theta = R(xi, eta)
...
# - Caminho CURVE -> CURVE -> STRAIGHT -> CURVE
...
# - Caminho LEFT -> RIGHT -> STRAIGHT -> LEFT -> RIGHT
def LRSLR(x, y, phi):
    # Fórmula 8.1 do paper
    xi = x + math.sin(phi)
    eta = y - 1.0 - math.cos(phi)
    rho, theta = R(xi, eta)
...
# - Caminho CURVE -> CURVE -> STRAIGHT -> CURVE -> CURVE
...
def generate_local_course(L, lengths, mode, maxc, step_size):
    ...
    for m, l, i in zip(mode, lengths, range(len(mode))):
        ...
        # Cálculo do comprimento remanescente
        ll = l - pd - d
...
# Retorna as coordenadas polares (r, theta) do ponto (x, y)
def R(x, y):
    r = math.hypot(x, y)
...
# Regula theta para ficar entre -pi e pi
def M(theta):
    phi = theta % (2.0 * PI)
...
def check_path(sx, sy, syaw, gx, gy, gyaw, maxc):
    ...
    for path in paths:
        ...
        # Checa distância do percurso
        d = [math.hypot(dx, dy)
             for dx, dy in zip(np.diff(path.x[0:len(path.x) - 1]), np.diff(path.y[0:len(path.y) - 1]))]
